...
The self-assessment workspace stores versioned local JSON under the ignored `runtime-data` directory. `/api/legora` supports snapshot, lock, comment, comment resolution, reviewer override, import and guided self-assessment actions. The browser surface supports local import and export.
